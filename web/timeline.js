...
if (els.clipOverlayToggle) {
  ...
  let savedValue = null;
  try { savedValue = localStorage.getItem(TIMELINE_OVERLAY_TOGGLE_KEY); } catch (_err) { /* storage disabled - keep default */ }
  ...
  els.clipOverlayToggle.addEventListener('change', () => {
    ...
    try { localStorage.setItem(TIMELINE_OVERLAY_TOGGLE_KEY, overlayEnabled ? '1' : '0'); } catch (_err) { /* storage disabled / quota - silently no-op */ }
    if (els.clipPlayer && !els.clipPlayer.paused && overlayShouldAnimate()) {
      startOverlayRaf();
