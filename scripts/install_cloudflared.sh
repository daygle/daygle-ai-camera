...
# old cloudflared in place forever. A matching install is left untouched, and
# DAYGLE_CLOUDFLARED_FORCE_UPDATE=1 still forces a reinstall. A missing or
# unreadable version string falls through to (re)install.
CURRENT_VERSION=""
if [[ -x "${INSTALL_PATH}" ]]; then
  CURRENT_VERSION="$("${INSTALL_PATH}" --version 2>/dev/null | grep -oE '[0-9]{4}\.[0-9]+\.[0-9]+' | head -n1 || true)"
fi
if [[ "${CURRENT_VERSION}" == "${VERSION}" && "${DAYGLE_CLOUDFLARED_FORCE_UPDATE:-0}" != "1" ]]; then
  echo "cloudflared ${VERSION} already installed at ${INSTALL_PATH}; leaving it unchanged."
  exit 0
fi
