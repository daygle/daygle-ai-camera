...
# background modules can reach them via main.<attr>.
...
# Pool A: recording-settings normalizers → app/recording_settings.py
...
# Pool A: AI subsystem cluster → app/ai_settings.py
...
# Pool A: settings payload validators → app/payload_validators.py
...
# Pool A: zone/schema normalizers → app/zone_schema.py
...
# Pool A: zone-detection orchestration → app/zone_detection.py
...
# Pool A: camera offline/health cluster → app/camera_health.py
...
# Pool A: live-snapshot rendering → app/live_snapshot.py
...
# Pool A: live-detection state cluster → app/detection_state.py
...
# Pool A: live-event debounce cluster → app/event_debounce.py
...
# Pool A: live-alert delivery cluster → app/alert_dispatch.py
...
# Pool A: live-detection status cluster → app/detection_status.py
...
# Pool A: recording-extension cluster → app/recording_extension.py
...
# Pool A: live-alert-monitor lifecycle + live-stream detection → app/live_monitor.py
...
# Pool A: pure stateless utility helpers → app/utils.py
...
# Pool A: camera diagnostics helper → app/diagnostics.py
from app.diagnostics import log_camera_diagnostic as log_camera_diagnostic
# Pool A: sound-monitor helpers → app/sound_monitor.py
...
# _on_sound_detected: no production caller; re-exported only to satisfy
# test_all_main_attr_references_resolve_on_app_main in test_api_router_split_invariants.py.
# Pool A: camera-instance helpers → app/camera_instance.py
...
# Pool A: media/ffmpeg-ffprobe helpers → app/media_utils.py
...
# Pool A: YOLO model-management helpers → app/model_management.py
...
# Pool A: database backup + recording-purge helpers → app/backup.py
