...
def validate_live_settings(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    object_detection_region_boost = normalize_bool_setting(merged.get('object_detection_region_boost'), False)
    # Adaptive cadence (Item 16). Defaults ON: a still camera slows its own
    # sampling, with a hard staleness floor so a stationary subject is still
    # detected. An operator who needs a fixed interval for a camera can turn
    # it off per camera - without this key the returned allowlist below would
    # strip the setting on save and the opt-out would silently do nothing.
    adaptive_detection_enabled = normalize_bool_setting(merged.get('adaptive_detection_enabled'), True)
    # Tiling grid: keep a valid 'off'/'CxR' string; anything unparseable -> 'off'.
    from app.region_detection import parse_tile_grid as _parse_tile_grid
    _tile_grid = _parse_tile_grid(merged.get('object_detection_tiling'))
    object_detection_tiling = f'{_tile_grid[0]}x{_tile_grid[1]}' if _tile_grid else 'off'
    try:
