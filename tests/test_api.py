...
# ---------------------------------------------------------------------------
# After Pool C elimination (PR #176), `alert_dispatch.py` imports
# `effective_push_notification_settings` and `PushNotificationService`
# at module top from their true homes (`app.config_facades` /
# `app.push_notifications`). The production call sites are module-level
# names in `app.alert_dispatch`, so patches must target `_ad.*` to
# intercept them.
# ---------------------------------------------------------------------------
