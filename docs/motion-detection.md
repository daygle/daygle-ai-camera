...
The **Profile Presets** row holds the **Cat / Small Animal**, **Balanced**, **Maximum Recall**, **Low CPU**, **Night / IR**, and **Fast Motion** starting points; custom presets can also be saved, updated, and deleted. **Day Preset** and **Night Preset** are separate selections, and the camera remembers both assignments. **Apply to Day** and **Apply to Night** load only the chosen side of a preset, so a camera can use (for example) Balanced detection during the day and Night / IR after dark without either selection overwriting the other. Nothing reaches the camera until you save.
