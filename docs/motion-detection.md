...
These remain the global defaults. Each camera can override the detection-performance settings from **Cameras → Edit Camera → Advanced → Day/Night Performance**. The Preset menu includes **Cat / Small Animal**, **Balanced**, **Maximum Recall**, **Low CPU**, **Night / IR**, and **Fast Motion** starting points; custom presets can also be saved, updated, and deleted. The camera stores separate Day and Night values for background detection, detection interval, ingest frame rate, confirmation frames/window/IoU, always-run object detection, region boost, tiling, periodic scans, motion-frame dimensions, and all motion tuning fields. Blank profile values inherit the global Live Performance value. Profile selection can be manual, schedule-based, or driven by ONVIF IR state. **Suggest Cat Profiles** provides a starting point for small, moving subjects: it enables frequent full-frame detection, confirmation, motion-region boosting, and stronger night scanning. Review the values for your camera and save; it does not enable automatic day/night switching.
