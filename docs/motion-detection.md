...
The **Profile Presets** row holds the **Cat / Small Animal**, **Balanced**, **Maximum Recall**, **Low CPU**, **Night / IR**, and **Fast Motion** starting points; custom presets can also be saved, updated, and deleted. **Apply to Day**, **Apply to Night**, and **Apply to Both** load the chosen preset into one slot or the other, so one preset can back the day profile while a different one backs the night profile - the untouched slot keeps whatever is already shown. Nothing reaches the camera until you save.
