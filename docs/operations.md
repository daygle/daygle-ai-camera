...
Use **Events** (`/events`) as the single activity feed for object, motion, and sound detections. Filter by event type and time range. Alerted events show a notification badge; when available, **Snapshot** opens an annotated still image and **Recording** opens the linked clip. The recording retention/auto-purge policy removes expired recording media and events that are no longer backed by retained media; a frameless sound event is removed when it ages out as well.
