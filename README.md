...
Deployments that need **both** the tunnel and LAN access (split DNS: LAN clients resolve the hostname to a local reverse proxy such as OPNsense HAProxy while external clients use the tunnel) can keep both ingress paths on the same origin. Enable **Serve LAN While Tunnel Is Active** on the Cloudflare Tunnel settings card (or set `server.tunnel_loopback_only: false` in `config.yaml`; a UI-saved value wins). The app then binds the configured `server.host` (typically `0.0.0.0`) even while the tunnel is active, so the connector and the LAN proxy both reach it. Add the LAN reverse proxy's IP to `auth.trusted_proxies` (Login Security card or config) and have the proxy send `X-Forwarded-Proto` (and `X-Forwarded-For`) so the dashboard's origin checks, audit log, and login rate-limiting keep working for LAN clients. Changing the binding takes effect on the next Daygle restart.
